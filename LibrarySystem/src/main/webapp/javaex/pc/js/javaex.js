...
//				$("#infinite-scroll-preloader-"+pageId).html(info.loadData);
			...
			upload : function() {
				...
				var type = "image";	// 上传类型
				var url = "/searchPic";		// 请求路径
				var inputId = "btn_file";	// <input type="file" />的id
				var isShowOptTip = false;// 是否显示操作提示
				var containerId = null;	// 图片容器id
				var uploadText = false;	// 上传图片文字提示
				var addImg = null;	// 默认添加图片
				var maxNum = 0;	// 文件上传数量上限
				var maxSize = 0;	// 单张图片的大小上限，单位KB，0或空为不限制
				var param = null;
				var imgUrl = null;	// 图片回显地址
				var dataType = "url";// 图片上传后的返回类型
